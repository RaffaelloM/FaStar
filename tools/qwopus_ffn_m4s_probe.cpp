// tools/qwopus_ffn_m4_probe.cpp — M=4 SPECULATIVE-DECODING FFN matvec probe.
// Validates fst_qwopus_ffn_m4.xclbin: dequant-once + native <4,8,8> MMUL across 4
// independent h-vectors (the SD amortization kernel).  Compares each of the 4 NPU
// matvec outputs to its own host mxfp4_matvec_f32 reference (argmax gate per h).
//
// Packet (host-packed uint8, 16928 B):
//   [32 B hdr(row_base,kc) | h0|h1|h2|h3 (4*2048=8192 BF16) | scales(512) | nibbles(8192)]
// 4 DISTINCT h-vectors (h_m[k] differs per m) so h-cross-contamination shows up.
// Output: 4*N_TILE held fp32 per tile = o[m*N_TILE + idx]; 4 matvec results.
//
// Usage: qwopus_ffn_m4_probe [dir] [shape] [scrange]
//   Env: FST_FFN_XCLBIN (default fst_qwopus_ffn_m4), FST_FFN_NOOP_XCLBIN (default
//   fst_qwopus_ffn_m4_noop).  shape=up|down, scrange=narrow (default).
#include <chrono>
#include <cmath>
#include <cstdint>
#include <cstdio>
...
constexpr int M_n = 16, MH = 4, G = 32, HDR = 32;
constexpr int H_BYTES   = MH * G * 32 * 2;   // 8192 (4 bf16 h-vectors)
constexpr int SC_BYTES = M_n * G;           // 512
constexpr int NB_BYTES = M_n * G * 16;      // 8192
constexpr int W_BYTES  = SC_BYTES + NB_BYTES;
constexpr int PKT_BYTES = HDR + H_BYTES + W_BYTES;  // 16928
constexpr int NT = 8;
constexpr int N_TILE_COMPILED = 2176;

struct Shape { const char* name; int N,K,GROUPS,KCHUNKS,NCHUNKS,N_TILE_USED; };
static const Shape SH_UP  = {"up",  17408, 5120, 160, 5, 136, 2176};
static const Shape SH_DOWN = {"down", 5120,17408, 544,17, 40,  640};
...
int main(int argc,char**argv){
    ...
    std::string full_stem=std::getenv("FST_FFN_XCLBIN")?std::getenv("FST_FFN_XCLBIN"):"fst_qwopus_ffn_m4s";
    std::string noop_stem=std::getenv("FST_FFN_NOOP_XCLBIN")?std::getenv("FST_FFN_NOOP_XCLBIN"):"fst_qwopus_ffn_m4s_noop";
    ...
    fprintf(stderr,"[probe] M=4 shape=%s N=%d K=%d GROUPS=%d KCHUNKS=%d NCHUNKS=%d N_TILE_USED=%d NPKT=%d\n",
    ...
    // pack input: 4 h's per packet
    const size_t IN_BYTES=(size_t)NT*NPKT*PKT_BYTES;
    const size_t OUT_PER_PKT=64; const size_t OUT_BYTES=(size_t)NT*NPKT*OUT_PER_PKT*4;
    ...
    for(int t=0;t<NT;t++) for(int nc=0;nc<NCHUNKS;nc++) for(int kc=0;kc<KCHUNKS;kc++){
        ...
        for(int m=0;m<MH;m++) pack_bf16_row(pkt+HDR+m*(G*32*2), &h[m][(size_t)kc*G*32], G*32); // h0..h3
    ...
    double tFull=run(kFull,"ffn_matvec_m4");
    const float* outF=boOut.map<float*>();
    std::vector<float> outFull(outF, outF+NT*NPKT*OUT_PER_PKT);
    double tNoop=run(kNoop,"ffn_matvec_m4_noop");
    // gather 4 NPU outputs (compiled-stride read, same as M=1 probe per h)
    // streamed: slot[t][pidx=nc*KCHUNKS+kc][m*16+nt*8+j]; sum over kc.
    std::vector<std::vector<float>> outNpu(MH, std::vector<float>(N, 0.0f));
    for(int t=0;t<NT;t++) for(int nc=0;nc<NCHUNKS;nc++) for(int nt=0;nt<2;nt++) for(int j=0;j<8;j++){
        int gn=t*N_TILE_USED+nc*M_n+nt*8+j; if(gn>=N) continue;
        for(int kc=0;kc<KCHUNKS;kc++){
            size_t pidx=(size_t)t*NPKT + (nc*KCHUNKS+kc);
            size_t base=pidx*OUT_PER_PKT;
            for(int m=0;m<MH;m++) outNpu[m][gn]+=outFull[base + m*16 + nt*8 + j];
        }
    }
    // argmax + rel per h
    double dma_gb=(double)IN_BYTES/tNoop/1e6, full_gb=(double)IN_BYTES/tFull/1e6, ratio=tFull/tNoop;
    printf("M4 shape=%s scrange=%s full_ms=%.2f noop_ms=%.2f ratio=%.2f full_gb=%.2f noop_gb=%.2f\n",
    ...
    fprintf(stderr,"TARGET: full < 42.8 ms (host M=4).  %s\n", (tFull<42.8)?"BEATS HOST":"LOSESES vs host");
    fprintf(stderr,"per-h cost: %.2f ms (full/4)\n", tFull/4.0);
    return any_fail?4:0;
}