...
int main(int argc,char**argv){
    ...
    std::string full_stem=std::getenv("FST_FFN_XCLBIN")?std::getenv("FST_FFN_XCLBIN"):"fst_qwopus_ffn_m4";
    std::string noop_stem=std::getenv("FST_FFN_NOOP_XCLBIN")?std::getenv("FST_FFN_NOOP_XCLBIN"):"fst_qwopus_ffn_m4_noop";
    ...
    const size_t OUT_BYTES=(size_t)NT*4*N_TILE_COMPILED*4;
    ...
    std::vector<float> outFull(outF, outF+NT*4*N_TILE_COMPILED);
    ...
    for(int m=0;m<MH;m++) for(int n=0;n<N;n++){int t=n/N_TILE_USED;int i=n%N_TILE_USED;
        outNpu[m][n]=outFull[(size_t)t*4*N_TILE_COMPILED + (size_t)m*N_TILE_COMPILED + i];}
    // argmax + rel per h
