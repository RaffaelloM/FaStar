...
# ── Packet geometry (must match fst_qwopus_ffn_m8_kernel.cc) ──────────────────
M_n       = 16
MH        = 8
...
BLK_BYTES = 17
W_BYTES   = M_n * G * BLK_BYTES   # 4352
PKT_BYTES = HDR + H_BYTES + W_BYTES   # 12576 (< 16 KB)
...
NT      = 16
N_TILE  = N // NT                 # 1088
NCHUNKS = N_TILE // M_n           # 68 n_chunks per tile
NPKT    = NCHUNKS * KCHUNKS      # 680 packets per tile
# BD repeat <= 255: 680 = 8 x 85
NPKT_O  = 8
NPKT_I  = NPKT // NPKT_O          # 85

SRC = os.environ.get("FST_FFN_SRC", str(PROJ_ROOT / "fst_qwopus_ffn_m8_kernel.cc"))
...
def _build(fn_name, xclbin_name, obj_name):
    ...
    def ffn_op(inp: In, out: Out):
        ...
        # depth=2 on the input fifo pipelines the shim DMA (the core acquires the
        # next packet while processing the current) — depth=1 made M=8 NT=16
        # DMA-bound at 2.66 GB/s (core stalled on acquire); depth=2 fits (output
        # 34.8 KB + 2x12.6 KB = 60 KB < 64 KB tile) and recovers the DMA rate.
        fins = [ObjectFifo(PKT_T, name=f"in{t}", depth=2) for t in range(NT)]
...
if __name__ == "__main__":
    ...
    if os.environ.get("FST_FFN_FN"):
        _build(os.environ["FST_FFN_FN"],
               os.environ.get("FST_FFN_XCLBIN", "fst_qwopus_ffn_m8"),
               os.environ.get("FST_FFN_OBJ", "fst_qwopus_ffn_m8.o"))
        ...
        _build("ffn_matvec_restream", "fst_qwopus_ffn_m8",      "fst_qwopus_ffn_m8.o")
    if which in ("both", "noop") and not os.environ.get("FST_FFN_FN"):
        _build("ffn_matvec_noop_stream", "fst_qwopus_ffn_m8_noop", "fst_qwopus_ffn_m8_noop.o")
