// fst_qwopus_ffn_m4_kernel.cc — M=4 SPECULATIVE-DECODING FFN matvec.
// Dequant the weight slice ONCE per (nt,g) (amortized across 4 h-vectors), then a
// NATIVE <4,8,8> MMUL (M=4 h-rows exact, 8 N-outputs/MMUL => 256 MMULs total, half
// the n4 <8,8,4> count) produces 4 independent matvec outputs per (nt,ks).  This is
// the dequant-amortization kernel: the SD goal is dequant-once (23.6 ms) + 256 MMULs
// (~19 ms) < 42.8 ms host M=4.
//
// Packet (M_n=16 = 2 nt tiles of 8 outputs; 4 h-vectors):
//   [hdr(32) | h0|h1|h2|h3 (4*2048=8192 BF16) | scales(512) | nibbles(8192)] = 16928 B
//   (NB: 16928 > 16 KB — the 16-KB packet wall is the known risk; tested below.)
// Output: 4*N_TILE held fp32 (one matvec result per h-vector); RMW across k_chunks.
// NO bfp16 emulation: <4,8,8> uses mac_4x8_8x8_bf16 (exact bf16, NO to_v64bfp16ebs8).
#include <aie_api/aie.hpp>
#include <stdint.h>
using namespace aie::operators;

constexpr int M_n = 16;     // outputs per packet (2 nt tiles of 8)
constexpr int MH  = 4;       // speculative-decoding M = 4 h-vectors
constexpr int G   = 32;
constexpr int HDR = 32;
constexpr int H_BYTES   = MH * G * 32 * 2;   // 8192 (4 h-vectors BF16)
constexpr int SC_BYTES = M_n * G;            // 512
constexpr int NB_BYTES = M_n * G * 16;       // 8192

// NATIVE <4,8,8> (mac_4x8_8x8_bf16, exact bf16, NO bfp16).  A[4,8]=4 h-rows,
// B[8,8]=8 outputs x 8 k, C[4,8]=4x8 matvec outputs (m=h-vector, j=output).
// M=4 exact (no wasted M-rows, unlike <8,8,4>'s broadcast).  NOTE: <4,8,8> has a
// known -4 register-underflow that is FATAL at M=1 but only "diluted" (cos 0.9988)
// at M>=4 — acceptable for SD argmax (verified separately).
...
// N_TILE_C = the compiled held-output width PER h-vector (one matvec result row).
// Hardcoded to the compiled geometry (2176 for gate/up; down reuses the same
// 2176-wide compiled buffer, writing the first 640).  The 4 h-results are laid out
// o[m*N_TILE_C + idx]; the host drains 4*N_TILE_C per tile.
constexpr int N_TILE_C = 2176;

extern "C" void ffn_matvec_restream(
    const uint8_t *__restrict pkt,
    float          *__restrict o)   // 4*N_TILE_C wide: o[m*N_TILE_C + idx]
{
    ...
    alignas(128) bfloat16 A_buf[32];   // [m=4, k=8]  (4 h-vectors x 8 k)
    alignas(128) bfloat16 B_buf[64];   // [k=8, j=8]  (8 outputs x 8 k)
    alignas(128) bfloat16 dq[256];     // [ks*64 + n*8 + kr] dequant of 8 outputs

    // 2 dequant tiles (nt 0,1; 8 outputs each) = M_n=16 outputs/packet.  Per nt:
    // ONE MMUL accumulator C reduces the 1024 k of this k_chunk (G=32 groups x 4
    // ks x 8 k) for all 4 h-vectors simultaneously (<4,8,8>: M=4 exact).  dequant
    // runs ONCE per (nt,g) and is reused across the 4 ks (the amortization).  1
    // live MMUL (sequential nt) avoids the 2-live-MMUL stack crash.
    for (int nt = 0; nt < 2; ++nt) {
        MMUL C;
        bool first = true;
        for (int g = 0; g < G; ++g) {
            dequant_group(scales, nibbles, nt, g, dq);   // ONCE per (nt,g) — amortized
            for (int ks = 0; ks < 4; ++ks) {             // 4 K-subs of 8 per group
                // A[m,k] = h_m[g*32 + ks*8 + k],  m=0..3 (4 h-vectors), k=0..7
                #pragma unroll
                for (int m = 0; m < 4; ++m) {
                    const bfloat16 *__restrict hm = h0 + (size_t)m * (G * 32);
                    #pragma unroll
                    for (int k = 0; k < 8; ++k)
                        A_buf[m * 8 + k] = hm[g * 32 + ks * 8 + k];
                }
                aie::vector<bfloat16, 32> A_op = aie::load_v<32>(A_buf);
                // B[k,j] = dq[ks*64 + j*8 + k]  (8 outputs j=0..7, k=0..7)
                #pragma unroll
                for (int k = 0; k < 8; ++k)
                    #pragma unroll
                    for (int j = 0; j < 8; ++j)
                        B_buf[k * 8 + j] = dq[ks * 64 + j * 8 + k];
                aie::vector<bfloat16, 64> B_op = aie::load_v<64>(B_buf);
                if (first) { C.mul(A_op, B_op); first = false; }
                else        { C.mac(A_op, B_op); }
            }
        }
        aie::vector<float, 32> cv = C.template to_vector<float>();   // cv[m*8+j]
        #pragma unroll
        for (int m = 0; m < 4; ++m)
            #pragma unroll
            for (int j = 0; j < 8; ++j) {
                float v = cv[m * 8 + j];
                int idx = row_base + nt * 8 + j;
                size_t off = (size_t)m * N_TILE_C + idx;
                if (kc == 0) o[off] = v; else o[off] += v;   // RMW across k_chunks
            }
    }
    event1();
}
    ...
    float          *__restrict o)
{
    ...
    for (int g = 0; g < G; ++g)
        for (int m = 0; m < MH; ++m) {
            aie::vector<bfloat16, 8> hv = aie::load_v<8>(h + (size_t)m * (G * 32) + g * 32);
            sink = (float)hv[0] + (float)hv[4];
        }
    for (int r = 0; r < M_n; r++) o[row_base + r] = 1.0f;
    (void)sink;
}
