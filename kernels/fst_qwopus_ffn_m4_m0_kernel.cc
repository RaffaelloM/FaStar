// fst_qwopus_ffn_m4_m0_kernel.cc — M=4 COMPUTE-FLOOR diagnostic.
// Full M=4 compute (4 h-vectors in A, 256 native <4,8,8> MMULs, dequant-once) but
// writes ONLY h0's result (m=0) to a 1x held output (fast 8.7 KB RMW).  Isolates the
// dequant+MMUL compute floor from the 4x output cost.  If this is ~42 ms, the compute
// is at the host line and the 4x output is the fundamental M=4 blocker.
#include <aie_api/aie.hpp>
#include <stdint.h>
using namespace aie::operators;
constexpr int M_n = 16, MH = 4, G = 32, HDR = 32;
constexpr int H_BYTES   = MH * G * 32 * 2;
...
extern "C" void ffn_matvec_restream(const uint8_t *__restrict pkt, float *__restrict o){
    ...
    alignas(128) bfloat16 A_buf[32], B_buf[64], dq[256];
    for (int nt = 0; nt < 2; ++nt) {
        ...
        aie::vector<float, 32> cv = C.template to_vector<float>();
        #pragma unroll
        for (int j = 0; j < 8; ++j) {          // m=0 ONLY (h0 result)
            float v = cv[j];
