...
static inline void dequant_group_const(const uint8_t *scales, const uint8_t *nibbles,
                                 int nt, int g, bfloat16 *dq) {
    // dq=1.0 isolation: fill ALL 256 slots (8 outputs x 32 k) with 1.0 in the same
    // [ks*64 + n*8 + kr] layout, so B[k,n]=1.0 for every (k,n) => C[0,n]=sum_k h[k]=Sh
    // identical across n => verifies the <8,8,4> B-packing + mmul + K-reduction +
    // accumulation + output extraction, INDEPENDENT of the real dequant values.
    ...
    float          *__restrict o)
{
    ...
    for (int nt = 0; nt < 2; ++nt) {                 // dequant tile 0 (rows 0..7), 1 (8..15)
        for (int nt4 = 0; nt4 < 2; ++nt4) {          // N-tile-of-4: 0..3 / 4..7 of the nt tile
            ...
            MMUL C;
            for (int g = 0; g < G; ++g) {
                dequant_group_const(scales, nibbles, nt, g, dq);   // 8 outputs × 32 k -> dq
