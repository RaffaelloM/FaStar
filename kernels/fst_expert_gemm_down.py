#!/usr/bin/env python3
"""FaStar MoE expert GEMM (DOWN projection) -- IRON/MLIR-AIE for XDNA2 (NPU2).

Single-core BF16 matmul C = A @ B for the MoE expert DOWN projection:
    A : [M, K] activations   (M = 32   , K = 2048 = moe_intermediate_size)
    B : [K, N] weights       (K = 2048 , N = 4096 = hidden_size)
    C : [M, N] output

Same Shim-DMA-only data flow as fst_expert_gemm.py; only the compile-time
dimensions differ (K and N are swapped vs the gate/up projection).
...
# DeepSeek V4 Flash down projection: K=intermediate=2048, N=hidden=4096.
M_FIX = 32  # activation bucket capacity
K_FIX = 2048  # moe_intermediate_size
N_FIX = 4096  # hidden_size
...
def fst_expert_gemm(
    ...
    *,
    ...
    # Custom 4D TensorAccessPatterns — same fix as fst_expert_gemm.py.
    # Decompose K into K-tiles (32) and K-elements (64) to keep all BD
    # dimension sizes under the 1023 AIE DMA hardware limit.
