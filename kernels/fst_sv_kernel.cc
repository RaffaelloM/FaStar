...
//
// fst_sv_kernel.cc -- Vectorized SV attention: GEMM tile + identity scale
//
// AIE2P (NPU2) target.
// GEMM tile: scores[m, k] × V[k, n] → out[m, n]
// Uses aie::mmul<4,8,8,bf16,bf16,accauto> with 2x2 expansion.
// Post-GEMM: identity (scores already scaled by QK).
...
// GEMM tile: scores[8,64] × V[64,64] → out[8,64]
void fst_sv_gemm(bfloat16 *scores, bfloat16 *v, bfloat16 *out)
{
    fst_matmul_bf16<8, 64, 64>(scores, v, out);
}

// Zero C accumulator: out[8,64] = 0
void fst_sv_zero(bfloat16 *out)
{
    fst_zero_vec<bfloat16, 8 * 64>(out);
}

// Identity scale - scores are already scaled by QK
void fst_sv_scale(bfloat16 *__restrict data, int32_t D)
{
    (void)data;
    (void)D;
}
