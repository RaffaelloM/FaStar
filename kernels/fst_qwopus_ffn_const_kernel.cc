// @variant fst_qwopus_ffn_const_kernel.cc
...
static inline void dequant_group(const uint8_t *scales, const uint8_t *nibbles,
    ...
    // dq=1.0 ISOLATION: write 1.0 to ALL 256 dq slots in the SAME [ks*64 + n*8 + kr]
    // strided layout the real dequant uses.  With B[k,n]=1.0 constant and A=h
    // broadcast, a CORRECT mmul gives C[0,n] = sum_k h[k] IDENTICAL for all 16 n.
    // Per-n variation => the mmul reads B from outside the intended dq slots (layout
    // bug) OR A is not broadcast OR cv[n]!=C[0,n].  (scales/nibbles unused.)
    (void)scales; (void)nibbles; (void)nt; (void)g;
    for (int i = 0; i < 256; ++i) dq[i] = (bfloat16)1.0f;
}
    ...
    for (int nt = 0; nt < 2; ++nt) {                 // N-tile 0 (rows 0..7), 1 (8..15)
        ...
        for (int g = 0; g < G; ++g) {
            ...
            for (int ks = 0; ks < 4; ++ks) {          // 4 K-subs of 8 per group
                ...
                aie::vector<bfloat16, 64> B_op = aie::transpose(
                    aie::load_v<64>(dq + ks * 64), 8, 8);   // single contiguous load
                C.mac(A_op, B_op);
            }
        }
