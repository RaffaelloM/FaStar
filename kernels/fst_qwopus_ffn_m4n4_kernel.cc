// fst_qwopus_ffn_m4n4_kernel.cc — M=4 SD FFN with the FAST <8,8,4> MMUL.
// 4 h-vectors in A rows 0..3 (rows 4..7 zeroed), dequant ONCE per (nt,g) reused across
// BOTH nt4 (NOT re-run per nt4 like n4v — removes the 2x dequant waste), 512 native
// <8,8,4> MMULs (mac_8x8_8x4_bf16, the FAST 0.074ms/MMUL path), 4 outputs (m=0..3).
// Measured pieces: dequant-once=23.6ms, <8,8,4> 512 MMULs~38ms -> target ~62ms vs
// host M=4 42.8ms.  (The <4,8,8> variant was 95ms: that MMUL is 4x slower/instr.)
#include <aie_api/aie.hpp>
#include <stdint.h>
using namespace aie::operators;
constexpr int M_n = 16, MH = 4, G = 32, HDR = 32;
constexpr int H_BYTES   = MH * G * 32 * 2;   // 8192 (4 h-vectors)
constexpr int SC_BYTES = M_n * G;
constexpr int NB_BYTES = M_n * G * 16;
constexpr int N_TILE_C = 2176;               // held output width per h-vector
using MMUL = aie::mmul<8, 8, 4, bfloat16, bfloat16, accfloat>;   // FAST mac_8x8_8x4_bf16
...
extern "C" void ffn_matvec_restream(const uint8_t *__restrict pkt, float *__restrict o){
    ...
    // all 32 groups.  Dequant-once-across-nt4 would need 2 live MMULs (C for both nt4
    // alive across g) -> stack crash; so dequant runs per nt4 (2x, no amortization).
    // The 4 h-vectors ride in A rows 0..3 free; this measures the 2x-dequant M=4 cost.
                ...
                for (int ks = 0; ks < 4; ++ks) {
                    ...
                    for (int m = 0; m < 8; ++m) {
                        if (m < 4) {
                            ...
                            #pragma unroll
                            for (int k = 0; k < 8; ++k) A_buf[m * 8 + k] = (bfloat16)0;
                        }
                    }
