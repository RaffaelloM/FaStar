...
HDR       = 32
MH = 4
H_BYTES   = MH * G * 32 * 2        # 8192 (4 h-vectors BF16, M=4 SD)
BLK_BYTES = 17
W_BYTES   = M_n * G * BLK_BYTES   # 8704
PKT_BYTES = HDR + H_BYTES + W_BYTES   # 16928 (4 h's + W)
OUT_PER_PKT = 64   # 4 h-vectors x 16 outputs/packet, STREAMED (no held)
...
OUT_T = np.ndarray[(OUT_PER_PKT,), np.dtype[np.float32]]  # per-packet slot, streamed
...
def _build(fn_name, xclbin_name, obj_name):
    ...
    def ffn_op(inp: In, out: Out):
        ...
        fos  = [ObjectFifo(OUT_T, name=f"o{t}",  depth=2) for t in range(NT)]

        def core(fin, fo, kM):
            for _p in range_(NPKT):
                p = fin.acquire(1)
                o = fo.acquire(1)                    # per-packet output slot (streamed)
                kM(p, o)
                fin.release(1)
                fo.release(1)
        ...
        with rt.sequence(np.ndarray[(NT * NPKT * PKT_BYTES,), np.dtype[np.uint8]],
                         np.ndarray[(NT * NPKT * OUT_PER_PKT,), np.dtype[np.float32]]) as (inp, out):
            rt.start(*ws)
            tg = rt.task_group()
            for t in range(NT):
                ...
                rt.drain(fos[t].cons(), out,
                         tap=TensorAccessPattern(tensor_dims=(NT * NPKT * OUT_PER_PKT,),
                                                 offset=t * NPKT * OUT_PER_PKT,
                                                 sizes=[NPKT_O, NPKT_I, OUT_PER_PKT],
                                                 strides=[NPKT_I * OUT_PER_PKT, OUT_PER_PKT, 1]),
