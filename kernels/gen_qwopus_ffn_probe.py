#!/usr/bin/env python3
"""gen_qwopus_ffn_probe.py — Stage-1.1 micro-probe IRON generator for the NPU
FFN lever (Qwopus3.6 dense SwiGLU FFN, M=1).

Measures the make-or-break unknown: NPU sustained weight-read bandwidth with
large BDs + a 20 KB read-only held input vector h.  One tile does a real
...
Emits fst_qwopus_ffn_probe.xclbin + _insts.bin.
"""
...
SRC = str(PROJ_ROOT / "fst_qwopus_ffn_probe_kernel.cc")
INC = [aie_config.cxx_header_path()]
OBJ = "fst_qwopus_ffn_probe.o"
...
def ffn_probe_op(inp_h: In, inp_w: In, out: Out):
    kM = ExternalFunction("ffn_matvec_rows", source_file=SRC,
...
if __name__ == "__main__":
    _emit(ffn_probe_op, "fst_qwopus_ffn_probe")
