// fst_qwopus_ffn_m4s_kernel.cc — M=4 STREAM-OUTPUT FFN matvec.
// Same as fst_qwopus_ffn_m4_kernel.cc (dequant-once + native <4,8,8> over 4 h's)
// BUT writes 64 partials/packet to a fresh per-packet output slot (NO held RMW,
// NO on-tile kc accumulation).  The host sums the KCHUNKS partials.  This removes
// the 34 KB held-output buffer (the on-tile-memory wall that made the held M=4
// kernel 96 ms).  Output fifo depth=NPKT (streamed); on-tile output state = 256 B/pkt.
...
// o = per-packet output slot, 64 floats = [m*16 + nt*8 + j]  (m=h-vector 0..3,
// nt=dequant tile 0..1, j=output 0..7).  NO RMW, NO kc check — fresh slot each packet.
    ...
    float          *__restrict o)
{
    ...
    for (int nt = 0; nt < 2; ++nt) {
        ...
        bool first = true;
        for (int g = 0; g < G; ++g) {
            dequant_group(scales, nibbles, nt, g, dq);
            for (int ks = 0; ks < 4; ++ks) {
                #pragma unroll
                for (int m = 0; m < 4; ++m) {
                ...
                for (int k = 0; k < 8; ++k)
                    #pragma unroll
                    for (int j = 0; j < 8; ++j) B_buf[k * 8 + j] = dq[ks * 64 + j * 8 + k];
                aie::vector<bfloat16, 64> B_op = aie::load_v<64>(B_buf);
                if (first) { C.mul(A_op, B_op); first = false; } else { C.mac(A_op, B_op); }
            }
        }
        ...
        for (int m = 0; m < 4; ++m)
            #pragma unroll
            for (int j = 0; j < 8; ++j)
                o[m * 16 + nt * 8 + j] = cv[m * 8 + j];   // fresh slot, no RMW
    }
    event1();
}
...
extern "C" void ffn_matvec_noop_stream(const uint8_t *__restrict pkt, float *__restrict o){
    ...
    for (int g = 0; g < G; ++g) for (int m = 0; m < MH; ++m) {
        aie::vector<bfloat16,8> hv=aie::load_v<8>(h+(size_t)m*(G*32)+g*32); sink=(float)hv[0]+(float)hv[4]; }
    for (int r = 0; r < 64; r++) o[r] = 1.0f;   // per-packet slot = 64 floats
    (void)sink; (void)row_base;
}
